...
public class BSTRecursive {
	...
	private int numNodes(BSTNode n) {
		int count=1;
		if (n.left!=null) count=count+numNodes(n.left);
		if (n.right!=null) count=count+numNodes(n.right);
		return count;
	}
	...
	private int numLeaves(BSTNode n) {
		...
		else return numLeaves(n.right)+numLeaves(n.left);
	}
	...
	private BSTRecursive greaterThan(BSTNode n, int v, BSTRecursive result) {
		...
		if (n.data>v) {
			if (result.root==null)  result.root=result.insert(n.data,result.root,null);
			else result.insert(n.data); 	
		}
	...
	private String toStringInOrder(BSTNode n) {
		if (n==null) return "";
		String left = toStringInOrder(n.left);
		if (left!="") left=left+" ";
		String right = toStringInOrder(n.right);
	...
	private boolean isBST(BSTNode n , int min, int max) {
	...
	private BSTNode findParentForwardDirection(BSTNode n, BSTNode d, BSTNode parent) {
		if (d.data<n.data)  return findParentForwardDirection(n.left,d,n);
		else if (d.data>n.data) return findParentForwardDirection(n.right,d,n);
		if (d.data==n.data) return parent;
		else return null;
	}
